...
int main(int argc, char **argv)
{
    ...
    for (auto section: ini.sections) {
        ...
        disconnected_clients.insert(std::pair<int, std::string>(sd, mac));
    ...
    while (true) {
        FD_ZERO(&readfds);
        for (auto it = disconnected_clients.begin(); it != disconnected_clients.end(); ++it) {
            sd = it->first;
            std::string mac = it->second;
            auto search = clients.find(sd);
